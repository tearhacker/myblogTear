...
### 验证接口(推荐)
- URL: `GET http://121.35.251.197/api/secret/v?k=你的密钥`
- 或: `POST http://121.35.251.197/api/secret/check` 参数 `key=你的密钥`
...
### 健康检查
- URL: `GET http://121.35.251.197/api/secret/ping`
...
int main() {
    ...
    // 真实服务器地址: http://121.35.251.197
    if (verifyKey("TEAR-2024-SECRET-DEMO-001", "http://121.35.251.197")) {
...
class TearAuth {
public:
    // 默认服务器地址: http://121.35.251.197
    static bool check(const char* key, const char* server = "http://121.35.251.197") {
        CURL* curl = curl_easy_init();
        if (!curl) return false;
...
// 使用示例:
// if (!TearAuth::check("TEAR-XXXX-XXXX-XXXX")) return -1;
```
...
### 创建密钥
- URL: `POST http://121.35.251.197/api/secret/TearGame/new_create`
- 参数:
  - `contact` (必填): 联系方式
